...
@repair_app.command("continuity")
def repair_continuity_cmd(
    dry_run: bool = typer.Option(
        False,
        "--dry-run",
        help="Preview hygiene assessment without quarantining artifacts.",
    ),
    as_json: bool = typer.Option(
        False,
        "--json",
    ...
    """Scan and repair stale continuity artifacts (ledger + journal).

    Artifacts are classified as:
      - **safe_stale_quarantine**: absent from plan and repaired claims → quarantine allowed
      - **blocking_divergence**: conflicts with plan or claims → blocks startup recovery
      - **corrupt_blocking**: unparsable or missing step identity → blocks startup recovery
      - **ambiguous_blocking**: migration/rename suspicion → blocks startup recovery
    ...
    # Build repaired current-branch claim step IDs (mirrors hygiene stage contract)
    branch = get_current_branch()
    claims_path = resolve_claims_path(plan_path)
    ...
    # Scan continuity artifacts
    repo_root = _resolve_repo_root_from_plan(plan_path)
    continuity_artifacts, corrupt_artifact_paths = scan_continuity_artifacts(repo_root)

    # Build plan step IDs
    plan_step_ids: set[str] = {step.id for phase in p.phases for step in phase.steps}

    # Build hygiene stage input
    quarantine_root = repo_root / ".vectl" / "continuity" / "quarantine"
    stage_input = StartupHygieneStageInput(
        current_branch=branch,
        current_plan_step_ids=tuple(plan_step_ids),
        repaired_current_branch_claim_step_ids=tuple(repaired_claim_ids),
        artifacts=continuity_artifacts,
        quarantine_root=str(quarantine_root),
    )

    # Run hygiene stage (assessment only, no quarantine side-effects yet)
    result = run_startup_hygiene_stage(stage_input=stage_input)

    # Apply quarantine if not dry-run
    if not dry_run:
        result = apply_quarantine(stage_result=result, quarantine_root=quarantine_root)

    # Load current manifest for reporting
    manifest_entries = load_quarantine_manifest(quarantine_root)

    # Build serializable payload
    blocked = [
        {
            "artifact_kind": a.artifact.artifact_kind,
            "original_path": a.artifact.original_path,
            "parsed_step_id": a.artifact.parsed_step_id,
            "classification": a.classification,
            "reason": a.reason,
            "blocks_startup_recovery": a.blocks_startup_recovery,
        }
    ...
    quarantined = [
        {
            ...
            "reason": e.reason,
            "quarantine_destination": e.quarantine_destination,
            "audit_timestamp": e.audit_timestamp,
        }
        for e in result.quarantine_manifest
    ]
    ...
    payload: dict[str, object] = {
        ...
        "policy": (
            "safe_stale_quarantine: quarantine artifacts absent from plan and repaired claims; "
            "blocking: corrupt, ambiguous, or divergent artifacts remain untouched and blocking"
        ),
        "safe_stale_rule": result.safe_stale_rule,
        "guardrails": list(result.guardrails),
        "plan_step_count": len(plan_step_ids),
        "repaired_claim_count": len(repaired_claim_ids),
        "artifacts_scanned": len(continuity_artifacts),
        "corrupt_artifacts": list(corrupt_artifact_paths),
        "classifications": [
            {
                "artifact_kind": a.artifact.artifact_kind,
                "original_path": a.artifact.original_path,
                ...
                "classification": a.classification,
                "reason": a.reason,
                "safe_stale_rule_satisfied": a.safe_stale_rule_satisfied,
                "blocks_startup_recovery": a.blocks_startup_recovery,
                "quarantine_destination": a.quarantine_destination,
            }
    ...
    mode = "[dry-run]" if dry_run else ""
    out.print(f"[bold]Repair continuity {mode}[/]")
    out.print(f"Branch: {branch}")
    out.print(f"Repo root: {repo_root}")
    out.print(f"Plan steps: {len(plan_step_ids)}")
    out.print(f"Repaired claims: {len(repaired_claim_ids)}")
    out.print(f"Artifacts scanned: {len(continuity_artifacts)}")

    if corrupt_artifact_paths:
        out.print(f"[red]Corrupt artifacts: {len(corrupt_artifact_paths)}[/]")
        for path in corrupt_artifact_paths:
            out.print(f"  [red]✗[/] {path}")

    if not result.assessments:
        out.print("[dim]No continuity artifacts found.[/]")
        return

    # Show safe stale summary first (actionable outcome)
    safe_stale = [a for a in result.assessments if a.classification == "safe_stale_quarantine"]
    if safe_stale:
        out.print(f"\n[green]Safe stale artifacts (quarantine eligible): {len(safe_stale)}[/]")
        for a in safe_stale:
    ...
    if result.blocked_assessments:
        ...
        for a in result.blocked_assessments:
            ...
            classification = a.classification
            reason = a.reason
            out.print(f"  [red]✗[/] {kind} {step}")
            out.print(f"      [dim]{classification}: {reason}[/]")

    if dry_run:
        out.print("\n[dim]Dry-run: no artifacts were moved. Run without --dry-run to apply.[/]")

    if quarantined and not dry_run:
        out.print(f"\n[green]Quarantined {len(quarantined)} artifact(s).[/]")
        out.print(f"[dim]Manifest: {quarantine_root / 'manifest.jsonl'}[/]")
