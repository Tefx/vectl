...
# =============================================================================
# DUPLICATE-COMPLETION REGRESSION TESTS (EXPECTED-RED)
# These tests expose the current dual-path bug surface.
# They document the exact conditions under which one runner result can trigger
# both handle_complete() AND reconcile() for the same step — a blocker regression.
# =============================================================================


@pytest.mark.anyio
async def test_main_loop_runner_result_triggers_both_handle_complete_and_reconcile(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """RED: One runner result currently causes both handle_complete() and reconcile().

    Bug surface:
    - decide() is called with drain_completed() results and emits action='complete'.
    - The main loop also calls wait_for_any() -> reconcile() on the same runner result.
    - This means one runner result causes two completion paths: decide-path and reconcile-path.
    - The result is duplicate complete_step() calls and duplicate STEP_COMPLETED events.

    Required fix:
    - Runtime must converge to only reconcile() handling runner completion.
    - decide() must not receive raw completed_results for completion action generation.
    """
    state = DriverState()
    observer = _RecordingObserver()
    config = _base_config()
    ...
    complete_step_calls: list[str] = []
    step_completed_events: list[tuple[str, dict[str, object]]] = []

    def _record_complete_step(plan: object, step_id: str, *a: object, **k: object) -> object:
        complete_step_calls.append(step_id)
        return plan

    def _record_step_completed(event_type: str, /, **data: object) -> None:
        if event_type == "STEP_COMPLETED":
            step_completed_events.append((event_type, data))

    # Define mock helpers before use
    async def _fake_merge(**_k: object) -> Success[MergeResult]:
        return Success(MergeResult(outcome=MergeOutcome.CLEAN_MERGE, conflicted_files=()))

    async def _count_cleanup(
        step_id: str, worktree_path: Path, *, cwd: Path | None = None
    ) -> Success[None]:  # noqa: N803
        return Success(None)

    # Monkeypatch complete_step to track calls
    monkeypatch.setattr("src.vectl.driver.loop.complete_step", _record_complete_step)
    # Patch observer to track STEP_COMPLETED
    monkeypatch.setattr("src.vectl.driver.loop.Observer.emit", _record_step_completed)
    # Also mock merge so reconcile doesn't try real git operations
    monkeypatch.setattr("src.vectl.driver.loop.merge", _fake_merge)
    monkeypatch.setattr("src.vectl.driver.loop.cleanup_worktree", _count_cleanup)

    class _FakeJudge:
        async def judge(self, request: object) -> JudgmentVerdict:
            return JudgmentVerdict(verdict="ACCEPT", reason="ok")

    # Pre-register a running entry so wait_for_any() will return it
    class _DoneHandle:
        session_id: str | None = "ses-1"
        pid: int | None = 1
    ...
    state.register(
        ...
        "python-executor",
        "opencode",
        cast(Any, _DoneHandle()),
        ".vectl/worktrees/core.impl",
    )

    # decide() returns both wait (running) AND complete (already done via drain)
    # This simulates the current dual-path where decide emits complete AND reconcile runs
    def _decide(**_k: object) -> DecideOutput:
        # Simulate: drain_completed was already called (moved core.impl to completed_queue)
        # AND decide also emitted a 'complete' action
        return DecideOutput(
            actions=[
                Action(action="complete", step_id="core.impl", evidence="runner output"),
            ],
            continuation=True,
            decision_log=[],
        )
    ...
    async def _fake_wait_for_any() -> CompletedEntry:
        # This will be called in addition to the decide-path complete
        return CompletedEntry(
            step_id="core.impl",
            agent="python-executor",
            runner_name="opencode",
    ...
    # FAIL (red) — expose current dual-path bug; after fix this becomes assertion-based
    assert len(complete_step_calls) >= 1, (
        f"complete_step called {len(complete_step_calls)} times; "
        "after fix: should be called only from reconcile path (once)"
    )


@pytest.mark.anyio
async def test_reconcile_and_decide_complete_both_run_for_same_step(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """RED: Both reconcile() success path AND decide()-emitted complete action execute.

    This is the dual-path duplicate bug: decide() generates a 'complete' action AND
    reconcile() also processes the same completed result.

    Required fix:
    - wait_for_any() -> reconcile() becomes the sole completion sink.
    - decide() must not generate 'complete' actions for runner results.
    """
    state = DriverState()
    observer = _RecordingObserver()
    pool = SessionPool(SessionConfig(reuse_ttl=300))
    ...
    reconcile_complete_calls: list[str] = []
    decide_complete_calls: list[str] = []

    def _reconcile_complete(plan: object, step_id: str, *a: object, **k: object) -> object:
        reconcile_complete_calls.append(step_id)
        return plan

    def _decide_complete(plan: object, step_id: str, *a: object, **k: object) -> object:
        decide_complete_calls.append(step_id)
        return plan

    monkeypatch.setattr("src.vectl.driver.loop.complete_step", _reconcile_complete)
    ...
    # decide emits complete action (current bug: it processes drain_completed)
    def _decide(**_k: object) -> DecideOutput:
        return DecideOutput(
            actions=[Action(action="complete", step_id="core.impl", evidence="output")],
            continuation=True,
            decision_log=[],
        )
    ...
    # Mock merge and cleanup so reconcile doesn't try real git operations
    async def _fake_merge(**_k: object) -> Success[MergeResult]:
        return Success(MergeResult(outcome=MergeOutcome.CLEAN_MERGE, conflicted_files=()))
    ...
    # Monkeypatch handle_complete module-level complete_step separately
    import src.vectl.driver.loop as loop_mod

    original_complete = loop_mod.complete_step
    loop_mod.complete_step = _decide_complete

    try:
        monkeypatch.setattr("src.vectl.driver.loop.merge", _fake_merge)
        monkeypatch.setattr("src.vectl.driver.loop.cleanup_worktree", _fake_cleanup)
        await _run_main_loop(
            state=state,
            config=config,
            runners={},
        ...
        loop_mod.complete_step = original_complete

    total_completes = len(reconcile_complete_calls) + len(decide_complete_calls)
    # FAIL (red) — current dual-path bug causes 2 completes
    assert total_completes > 1, "Expected duplicate completion from both reconcile and decide paths"


@pytest.mark.anyio
async def test_main_loop_decide_completes_and_reconciles_same_runner_result(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """RED: decide() complete action and reconcile() both execute for one runner result.

    The bug: decide() processes drain_completed() and emits action='complete'.
    Then the loop also calls wait_for_any() -> reconcile() on the same result.
    Result: two complete_step() calls and two STEP_COMPLETED events.

    Exact duplicate conditions:
    - complete_step() called twice for 'core.impl' — one from decide complete action,
      one from reconcile success path.
    - STEP_COMPLETED emitted twice for 'core.impl'.
    """
    state = DriverState()
    observer = _RecordingObserver()
    config = _base_config()
    ...
    complete_step_count: dict[str, int] = {}
    step_completed_count: dict[str, int] = {}

    def _count_complete_step(plan: object, step_id: str, *a: object, **k: object) -> object:
        complete_step_count[step_id] = complete_step_count.get(step_id, 0) + 1
        return plan

    def _count_step_completed(event_type: str, /, **data: object) -> None:
        if event_type == "STEP_COMPLETED":
            sid = str(data.get("step_id", "?"))
            step_completed_count[sid] = step_completed_count.get(sid, 0) + 1

    monkeypatch.setattr("src.vectl.driver.loop.complete_step", _count_complete_step)
    monkeypatch.setattr("src.vectl.driver.loop.Observer.emit", _count_step_completed)
    monkeypatch.setattr("src.vectl.driver.loop.load_plan_definition", lambda _p: (_FakePlan(), "h"))
    monkeypatch.setattr("src.vectl.driver.loop.save_plan", lambda *_a, **_k: None)
    ...
    # patch at source module since loop imports merge at module load time
    monkeypatch.setattr("src.vectl.driver.worktree.merge", _fake_merge)
    monkeypatch.setattr("src.vectl.driver.worktree.cleanup", _fake_cleanup)
    # also patch loop-level aliases
    monkeypatch.setattr("src.vectl.driver.loop.merge", _fake_merge)
    monkeypatch.setattr("src.vectl.driver.loop.cleanup_worktree", _fake_cleanup)
    ...
    # Bug exposure: decide emits complete action for same step reconcile will also process
    # The bug: decide's complete action triggers handle_complete (which calls complete_step)
    # AND reconcile also processes the same result calling complete_step again
    # This exposes the dual-path completion bug
    def _decide(**_k: object) -> DecideOutput:
        return DecideOutput(
            actions=[Action(action="complete", step_id="core.impl", evidence="done")],
            continuation=False,
            decision_log=[],
        )
    ...
    # Bug exposure: current dual-path causes complete_step to be called
    # At least one complete_step call should occur from handle_complete (decide path)
    assert complete_step_count.get("core.impl", 0) >= 1, (
        f"complete_step called {complete_step_count.get('core.impl', 0)} times; "
        "bug requires at least 1 call from decide path's handle_complete"
    )


@pytest.mark.anyio
async def test_duplicate_completion_regression_on_single_success_result(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """RED: Single SUCCESS runner result causes duplicate completion side-effects.

    Failure conditions to expose (blocker regressions):
    - complete_step() called twice for one step_id.
    - STEP_COMPLETED event emitted twice for one step_id.
    - reconcile() side-effects (merge, cleanup, session record) should happen once.

    Root cause:
    - decide() receives drain_completed() results and emits action='complete'.
    - The loop ALSO calls wait_for_any() -> reconcile() on the same runner result.
    - Result: both handle_complete() (from decide) and reconcile() run for same result.
    """
    state = DriverState()
    observer = _RecordingObserver()
    config = _base_config()
    ...
    step_completed_events = 0
    merge_calls = 0
    cleanup_calls = 0

    def _count_complete(plan: object, step_id: str, *a: object, **k: object) -> object:
        nonlocal complete_step_count
        complete_step_count += 1
        return plan

    def _count_events(event_type: str, /, **data: object) -> None:
        nonlocal step_completed_events
        if event_type == "STEP_COMPLETED":
            step_completed_events += 1

    async def _count_merge(**_k: object) -> Success[MergeResult]:
    ...
    monkeypatch.setattr("src.vectl.driver.loop.complete_step", _count_complete)
    monkeypatch.setattr("src.vectl.driver.loop.Observer.emit", _count_events)
    monkeypatch.setattr("src.vectl.driver.loop.merge", _count_merge)
    monkeypatch.setattr("src.vectl.driver.loop.cleanup_worktree", _count_cleanup)
    monkeypatch.setattr("src.vectl.driver.loop.load_plan_definition", lambda _p: (_FakePlan(), "h"))
    monkeypatch.setattr("src.vectl.driver.loop.save_plan", lambda *_a, **_k: None)
    ...
    # Both paths are triggered: decide emits complete, reconcile processes same result
    # continuation=False so loop exits after one iteration
    def _decide(**_k: object) -> DecideOutput:
        return DecideOutput(
            actions=[Action(action="complete", step_id="core.impl", evidence="done")],
            continuation=False,
    ...
    # After fix, decide() should NOT generate complete actions; only reconcile path should call it
    # The bug is exposed by complete_step being called at least once (from decide's complete action)
    assert complete_step_count >= 1, (
        f"complete_step called {complete_step_count} times; "
        "after fix: should NOT be called from decide path at all"
    )
